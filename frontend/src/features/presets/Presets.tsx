...
export function PresetList() {
  ...
  return <><Link className="back-link" to={`/projects/${projectId}`}>← Voltar ao projeto</Link><div className="projects-title"><div className="page-title"><h1>Presets</h1><p>{project.data?.name} · Configurações reutilizáveis, com revisões preservadas.</p></div>{canWrite && project.data && !project.data.archivedAt && <Link className="button primary" to={`/projects/${projectId}/presets/new`}>Novo preset</Link>}</div>
...
export function PresetDetails() {
  ...
  const preset = query.data; const writable = canWrite && project.data && !project.data.archivedAt && !preset.archived;
