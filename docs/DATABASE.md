...
PostgreSQL, UUIDs, datas UTC/timestamptz, FKs explícitas. Projects e TestSuites implementadas pelas migrações InitialProjects e AddTestSuites; as demais entidades continuam planejadas. Não usar EnsureCreated como substituto de migrações na aplicação.
...
- TestCase: Id, TestSuiteId, Name, StableKey, CatalogVersion, Tags, Status. Chave estável única dentro da suíte.
- Environment (nome CLR ProjectEnvironment): Id, ProjectId, Name (Development/Staging/Production), BaseUrl, Enabled. Unique(ProjectId, Name). Credenciais ficam fora do banco/snapshot inicial.
...
Projects 1:N TestSuites já implementado; os demais relacionamentos serão adicionados incrementalmente.
