...
Presets disponíveis na versão 0.9.0: configurações por projeto com nome, descrição, revisões preservadas, pesquisa, paginação e arquivamento. Criação/edição reutiliza o assistente de execução; salvar não cria nem executa testes. Reutilizar exige revisar a configuração atual revalidada e confirmar criação de Pending. Alterações de catálogo ou preset após revisão exigem revisar novamente. Origem e revisão são preservadas na execução. Exemplos de nomes: Regression Full e Debug Login; Smoke Production continua bloqueado até autorização específica. Próxima etapa: Autenticação e permissões, seguida de CI/CD e Docker/PostgreSQL.

Dashboard disponível em /dashboard (0.8.0): totais por estado, resultados finais por caso/navegador, taxa de sucesso, duração média da última tentativa avaliada, evolução diária, últimas oito execuções/falhas e oito casos com mais recuperações após retry. Período de até 90 dias UTC e filtro por projeto. Casos sem detalhes antigos não são reconstruídos; cobertura é informada. Instabilidade significa recuperação dentro da mesma execução, sem comparações avançadas entre execuções. Próxima etapa: Presets; autenticação, CI/CD e Docker/PostgreSQL seguem pendentes nessa ordem.
